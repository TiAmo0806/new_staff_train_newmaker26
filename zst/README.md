...
单帧YOLO结果不再写终端，只在OpenCV窗口中通过检测框和`YOLO L-R`实时显示；
...
1. 每个角度连续观察 `vote_frames_per_stage` 帧，数字至少命中
   `min_hits_per_stage` 帧才属于稳定候选。
2. 同一个数字在同一帧即使出现多个重复框，也只取置信度最高的一个框计票一次。
3. 第一角度必须先凑齐 `digits_per_view` 个稳定、未保存且不重复的新数字。
4. 数量未达到门槛时整批拒绝，本轮一个数字也不会写入正式数组。
5. 数量达到门槛后，按每个数字在多帧中的平均检测框中心X坐标从左到右排序，
   然后一次性写入正式数组。
6. 已保存4个、数组只剩1个空位时，下一角度要求自动从4降为1。
...
workflow:
  ...
  min_hits_per_stage: 6
  digits_per_view: 4
```

例如实际画面从左到右是 `2 4 5 1 3`。第一轮投票只有 `2、5、1` 达标时，程序会
输出“稳定新数字=3，要求=4，本轮不保存”，正式数组仍是 `[0 0 0 0 0]`。等同一
角度中的 `2、4、5、1` 全部稳定后，程序依据平均X排序并整批保存：

```text
[数字角度核对] 数量达标，从左到右=2(...) -> 4(...) -> 5(...) -> 1(...)
[数字缓存] 已保存=4/5，当前数组=[2 4 5 1 0]，请切换角度识别剩余数字
```

第二角度识别到最后的 `3` 后，最终数组才变成 `[2 4 5 1 3]`。这部分只改变
YOLO结果的分析和缓存方式，不改变A组6字节结果、B组5字节数字消息及任何CMD。
...
```text
[B组投票] 当前阶段是否新增稳定结果
[数字角度核对] 稳定数字数量、命中次数、平均X和从左到右顺序
[数字缓存] 本角度整批提交后的正式数字数组
