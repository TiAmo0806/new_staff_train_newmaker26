...
```text
[数字顺序核对] 通过，15/20帧从左到右一致=2 -> 4 -> 5 -> 1
...
- A组采用完整排列投票：首次同帧识别出黄豆、绿豆、白芸豆后才启动20帧窗口；完整帧
  按检测框中心X从左到右形成一张顺序票。同一种完整顺序至少出现15帧，才一次性写入
...
[A组豆子顺序核对] 通过，15/20帧从左到右一致=绿豆 -> 黄豆 -> 白芸豆
...
3. 只有同一帧恰好出现4个不同数字时，才按检测框中心X从左到右形成一张顺序票；
   少于4个或误检出5个的帧均不计票。
...
例如实际可见画面从左到右是 `2 4 5 1`。20帧中有15帧完整出现并保持这个顺序，
...
```text
[B组投票] 当前阶段是否新增稳定结果
[数字顺序核对] 完整4目标帧数、最高一致顺序票和最终从左到右顺序
