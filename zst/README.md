...
稳定结果生成后先冻结为待发送包。若Linux串口没有完整写出，视觉流程不会推进，并在
后续处理帧中重试同一业务包；整帧完整写出后才推进流程并保存断点。当前仍不等待电控
确认，因此“Linux串口写入成功”不等于“电控业务代码已经解析成功”。联调时必须同时
观察视觉TX十六进制日志和电控接收变量；如果以后需要端到端确认与丢包重发，再单独
恢复SEQ+ACK协议。
...
```text
1. 三豆直接识别，或识别前两豆并推测place3；按从左到右保存位置1~3，生成CMD=0x10和3字节DATA。
...
- A组首次同帧识别出至少两个不同豆子后启动20帧窗口。恰好出现3个不同豆子的帧，
  按检测框中心X从左到右形成“三豆直接票”；同一完整排列达到12帧就直接保存。
- 恰好出现2个不同豆子的帧单独形成“前两豆推测票”，不与三豆帧混算。同一前两豆
  排列达到15帧时，将它们写入place1、place2，并把三种豆子中缺少的一种固定写入
  place3。三豆直接结果优先；两条路径都不达标就清空本轮临时票并重新采集。
- 两豆推测成立的现场前提是：调节相机角度后，画面里的两个豆子确实是物理位置
  place1和place2；如果实际露出的是place1和place3，推测位置会错误。
...
```text
[A组豆子顺序核对] 直接通过，12/20帧三豆从左到右一致=绿豆 -> 黄豆 -> 白芸豆
[A组豆子推测] 通过，15/20帧前两豆从左到右一致=绿豆 -> 白芸豆，推测place3=黄豆
...
workflow:
  ...
  min_complete_bean_order_frames: 12 # A组三豆直接排列阈值
  min_inferred_bean_order_frames: 15 # A组前两豆推测place3阈值
  min_consistent_order_frames: 15    # A/B数字4目标完整顺序阈值
```
...
本机串口未完整写出时会冻结流程并重试同一结果；完整写出后推进状态并保存断点，
不等待电控回复，也不会对已经完整写出的业务结果自动重发。
...
按键或电控命令正常关闭相机时，程序使用`CameraPause`暂停采集并保留SDK句柄；再次
开启时使用`CameraPlay`恢复，避免反复销毁、枚举和初始化USB相机导致主循环卡住。
只有连续取帧失败达到阈值或程序退出时才完整执行`CameraStop -> CameraUnInit`。
重连等待采用定时调度，不再在主线程中阻塞1秒；等待期间仍处理`cv::waitKey()`，因此
窗口保持响应，仍可按 `q` 或 `ESC`退出。
...
模拟发送会被视为本机完整写出，无ACK状态机会正常继续并保存断点。
