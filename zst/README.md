...
1. 连续观察 `vote_frames_per_stage` 帧，数字至少命中
   `min_hits_per_stage` 帧才属于稳定候选。
2. 同一个数字在同一帧即使出现多个重复框，也只取置信度最高的一个框计票一次。
3. 必须恰好得到4个稳定、互不重复的数字；少于4个或出现5个都整批拒绝。
4. 四个数字按多帧平均检测框中心X坐标从左到右排序，一次性写入place1~place4。
...
该推理流程不需要切换第二角度，也不改变A组6字节结果、B组5字节数字消息及任何CMD。
