...
视觉识别结果使用固定命令长度格式，并增加1字节SEQ用于ACK匹配和重复帧去重：
...
- `SEQ`：结果序号1~255；ACK超时重发时保持不变，0不使用。
- `DATA`：全部为 `uint8_t`，不使用ByteConverter。
- `CRC_L/CRC_H`：覆盖A6、CMD、SEQ和全部DATA，低字节先发。
- `VERSION、TEAM、SESSION、LENGTH` 不发送；只保留可靠通信必需的 `SEQ`。
...
电控正确接收并接受视觉结果后，必须返回固定7字节ACK：

```text
5A 80 SEQ ORIGINAL_CMD STATUS CRC_L CRC_H
```

| 字节下标 | 类型 | 含义 |
|---:|---|---|
| 0 | `uint8_t` | 接收方向帧头`0x5A` |
| 1 | `uint8_t` | ACK命令`0x80`，与camera_state的0/1区分 |
| 2 | `uint8_t` | 原视觉结果中的SEQ |
| 3 | `uint8_t` | 原视觉结果中的CMD |
| 4 | `uint8_t` | STATUS：0接受，1数据非法，2业务阶段错误 |
| 5 | `uint8_t` | CRC16低字节 |
| 6 | `uint8_t` | CRC16高字节 |

ACK的CRC覆盖前5字节。电控只有在结果CRC正确、DATA长度正确并且已经把结果保存到
自己的有效变量后，才能回复`STATUS=0`。结果CRC错误时不要回复，由视觉300ms超时后
使用相同CMD、SEQ和DATA重发。

```c
/*
 * 向视觉电脑确认一条识别结果。
 * sequence和original_cmd必须从刚刚正确解析的视觉结果帧中原样取出，不能自己递增。
 * status=0表示结果已经被电控保存并接受；其他状态表示拒绝，视觉不会推进阶段。
 */
void Vision_SendResultAck(uint8_t sequence,
                          ...
                          uint8_t status)
{
    uint8_t frame[7] = {
        0x5AU,          /* [0] 电控发给视觉的统一帧头 */
        0x80U,          /* [1] 结果ACK命令 */
        sequence,       /* [2] 被确认结果的SEQ */
        original_cmd,   /* [3] 被确认结果的原CMD */
        status,         /* [4] 0=接受，1=数据非法，2=阶段错误 */
        0x00U,          /* [5] CRC低字节占位 */
        0x00U           /* [6] CRC高字节占位 */
    };

    /* CRC只覆盖前5个有效字段，不把末尾两个CRC占位字节计入。 */
    const uint16_t crc = Vision_CRC16(frame, 5U);
    frame[5] = (uint8_t)(crc & 0x00FFU);          /* CRC低字节先发送 */
    frame[6] = (uint8_t)((crc >> 8U) & 0x00FFU); /* CRC高字节后发送 */

    /* 替换成电控工程实际使用的UART句柄；USB CDC则换成CDC发送函数。 */
    (void)HAL_UART_Transmit(&huart1, frame, sizeof(frame), 20U);
}
```

电控还必须保存“最后已经接受的SEQ和CMD”。相同SEQ和CMD再次到达时，说明视觉没有收到
上一次ACK：电控不要重复执行或重复改变业务状态，只需再次发送同一个ACK。

例如确认A组第一条数字结果`CMD=0x10、SEQ=1`时，ACK为：

```text
5A 80 01 10 00 61 EB
```

联调时按以下顺序验证，不能只测试“正常立即ACK”一种情况：

1. 电控暂时不回ACK：视觉应每`ack_timeout_ms`使用同一SEQ重复发送，不能进入下一阶段。
2. 电控回复错误SEQ或错误原CMD：视觉应打印“不匹配”并继续等待。
3. 电控回复正确ACK：视觉只推进一次阶段，并把`next_sequence`写入断点。
4. 电控把同一ACK再发一次：视觉应把它作为重复ACK忽略，不能再次推进。
5. 相机关闭时丢失ACK：视觉仍应在相机关闭状态下继续重发和接收ACK。

算法端由 `VirtualSerial::receiveEvent()` 非阻塞拆分相机控制帧和ACK帧。串口一次只
...
`5A 00 3F DE` 后关闭相机，但串口、ACK重发和程序主循环继续运行。再次收到开启命令
...
```yaml
serial:
  simulated: true       # 结果只打印，并自动生成本地成功ACK
runtime:
  mode: "debug"         # 启动后直接打开相机
```

调试模式并没有删除串口和ACK代码：如果使用 `simulated: false`，视觉仍会正常接收
电控ACK，只是不再让电控控制相机开关。若填写了 `debug/competition` 之外的字符串，
程序会报告配置错误并退出，防止比赛时因为拼写错误意外打开相机。

TeamA 消息顺序：

```text
1. CMD=0x10，SEQ=1，DATA=[place1, place2, place3, place4, place5]
2. 收到匹配ACK后，CMD=0x11，SEQ=2，DATA=[bean1, bean2, bean3]
```

A组不再发送FinalResult，也不发送bean_target_place。示例：

```text
数字：A6 10 01 01 02 03 04 05 56 40
豆子：A6 11 02 01 02 03 CD 4D
```
...
```text
绿豆（SEQ=1）： A6 20 01 02 77 D3
数字（SEQ=2）： A6 21 02 04 01 02 05 03 1F 20
黄豆（SEQ=3）： A6 20 03 01 5C D2
白芸豆（SEQ=4）：A6 20 04 03 46 BC
```

算法端使用 `VisionTxPacket` 分别保存`command`、`sequence`和`data`；
`VirtualSerial` 再按`[A6][CMD][SEQ][DATA...][CRC]`封帧发送。
...
[发送准备] 当前电脑模式、CMD、SEQ和十进制DATA
[WaitingAck] 首次发送、超时重发、ACK匹配及阶段推进
...
当前已经实现结果ACK闭环：视觉生成结果后进入`WaitingAck`，不会继续累计下一阶段；
只有收到SEQ、原CMD都匹配且STATUS=0的ACK后，才推进阶段并保存断点。ACK超过
`serial.ack_timeout_ms`仍未到达时，视觉会原样重发同一结果。`write()`成功只表示
Linux接收了待发送字节，不再被当成电控已经接受结果。
...
只关闭或断开相机、但不退出程序时，工作流状态仍保存在内存中：例如A组数字结果
已经收到ACK，重新连接相机后会继续等待豆子。若使用 `q`、`ESC` 或强制停止使程序
...
断点只在收到匹配且STATUS=0的ACK后保存；仅仅`sendPacket()`成功不会推进阶段。
断点格式同时保存`next_sequence`，重启后继续使用正确的新SEQ。默认进度文件位于
项目根目录下的`runtime/workflow_progress.txt`。

A组示例：

```text
识别并发送5个数字
    -> WaitingAck，可能使用同一SEQ重发
    -> 收到正确ACK后保存 waiting_beans
    -> 关闭程序/相机
    -> 再次启动
    -> 自动恢复 waiting_beans
    -> 直接识别并发送3个豆子
...
```text
[WorkflowResume] 已恢复: mode=team_a, stage=waiting_beans, next_seq=2, 豆子=0/3, 箱位=5/5
```
...
注意：`serial.simulated: true`时没有真实C板，程序会在模拟发送后自动生成本地
STATUS=0 ACK，让离线算法调试能够继续完整状态机；测试过程同样会保存断点。
