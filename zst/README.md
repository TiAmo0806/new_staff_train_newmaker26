...
```text
FPS 12.5 | CAP 3.2 ms | VISION 72.4 ms
```

终端每秒也会输出一次平均值：

```text
[Performance] FPS=12.5，capture=3.2ms，vision=72.4ms
```

- `FPS`：相机取帧、视觉处理和工作流合计后的实际处理帧率。
- `CAP` / `capture`：等待并取得相机一帧的耗时。
- `VISION` / `vision`：YOLO、可选SVM、任务规划和调试图绘制耗时。
- 如果 `VISION` 很高，瓶颈主要在CPU推理；可比较 `intra_op_threads: 0/2/4`。
- 如果 `CAP` 很高或连续失败，优先检查USB连接、虚拟机USB直通、相机驱动和曝光。
