/**
 * @file    vision.c
 * @brief   抓豆分拣 C板 - 视觉通信协议实现 (USB CDC)
 * @note    与 VisionNode (NUC) 通过 USB CDC 通信
 *
 * 工作流程:
 *   1. CDC_Receive_FS() → vision_cdc_rx_callback() 将数据存入 rx_buf
 *   2. vision_task() 从 rx_buf 中查找完整帧 → CRC校验 → 解析
...
static int vision_scan_frame(const uint8_t *buf, int buf_len,
...
{
    memcpy(vision_real.digits, &frame[1], 5);
...
void vision_cdc_rx_callback(uint8_t *buf, uint32_t len)
{
    for (uint32_t i = 0; i < len; i++)
    {
        ...
        {
            vision_real.rx_buf[vision_real.rx_len++] = buf[i];
        }
        else
        {
            // 缓冲区满, 丢弃最早的一半
...
{
    uint8_t frame[20];
    int end = vision_scan_frame(vision_real.rx_buf, vision_real.rx_len,
                                header, size, frame);
    if (end > 0)
    {
