...
public:
    ...
    // 可变长数据包（用于发送检测结果等）
    // 帧格式 (示例实现)：帧头(0x5D) + 长度(1) + 有效载荷(n) + CRC16(2)
    static const uint8_t FRAME_HEADER_DATA   = 0x5D;  // 数据包帧头
    ...
    /**
     * @brief 发送可变长数据包给电控（带重试机制）
     *
     * 帧格式（示例）：帧头(0x5D) + 长度(1) + 有效载荷(n) + CRC16(2)
     * 长度为有效载荷字节数（不包含 CRC 与帧头/长度字节）
     *
     * @param payload 要发送的有效载荷
     * @param maxRetries 最大重试次数，默认3次
     * @return 是否成功
     */
    bool sendDataPacket(const std::vector<uint8_t> &payload, int maxRetries = 3);
