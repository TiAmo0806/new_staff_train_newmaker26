/**
 * @file CRC16.cpp
 * @brief CRC16-CCITT 查表法实现 (from dart project)
 */
...
uint32_t Verify_CRC16_Check_Sum(const uint8_t * pchMessage, uint32_t dwLength)
{
  ...
  uint16_t w_expected = Get_CRC16_Check_Sum(pchMessage, dwLength - 2, CRC16_INIT);
...
{
  if (dwLength <= 2) return;
  uint16_t w_crc = Get_CRC16_Check_Sum(pchMessage, dwLength - 2, CRC16_INIT);
