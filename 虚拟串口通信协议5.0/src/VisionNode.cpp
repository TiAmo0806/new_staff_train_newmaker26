...
int VisionNode::Run()
{
    ...
    while (true)
    {
        ...
        if (!config_.simulate)
        {
            ...
            // 豆子稳定 → 发 0xAA
            if (beanStableCount_ >= STABLE_THRESHOLD && !beanSent_)
            {
            ...
            // 数字稳定 → 发 0xCC
            if (digitStableCount_ >= DIGIT_STABLE_THRESHOLD && !digitSent_)
            {
        ...
        // 4. 信息叠加（显示稳定状态）
        {
            ...
            if (config_.simulate)
                ...
                stableInfo = "豆子:" + std::to_string(beanStableCount_) + "/" + std::to_string(STABLE_THRESHOLD)
                           + " 数字:" + std::to_string(digitStableCount_) + "/" + std::to_string(DIGIT_STABLE_THRESHOLD);
