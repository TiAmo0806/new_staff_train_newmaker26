...
int main(int argc, char** argv)
{
    ...
    // ---- 数字识别连续帧稳定性检查 ----
    int      lastDetectedDigit    = -1;      // 上一帧检测到的数字（用于判断连续性）
    int      consecutiveDigitCount = 0;      // 连续检测到相同数字的帧数
    ...
    while (true) {
        ...
        if (beansRecorded && !matchingDone && !waitingForBeanConfirm) {
            ...
            if (bestDigit >= 1 && bestDigit <= 5) {
                if (bestDigit == lastDetectedDigit) {
                    // 与上一帧相同，累加计数
                    consecutiveDigitCount++;
                } else {
                    // 数字变化，重置计数
                    lastDetectedDigit = bestDigit;
                ...
                // 连续 N 帧一致且与目标匹配，才发送确认信号
                if (consecutiveDigitCount >= cfg.digitStableThreshold &&
                    static_cast<uint8_t>(bestDigit) == targetNum) {
                    serial.sendMatchSignal(cfg.serialMaxRetries);
                    ...
                    std::cout << "[INFO] 匹配！Phase " << cyclePhase
                              ...
                              << " 检测=" << bestDigit
                              << " (连续" << cfg.digitStableThreshold
                              << "帧稳定) — 等待3箱豆子确认..." << std::endl;
                } else if (consecutiveDigitCount >= cfg.digitStableThreshold &&
                           static_cast<uint8_t>(bestDigit) != targetNum) {
                    // 连续 N 帧一致但不匹配目标，重置等待下一轮
                    std::cout << "[INFO] 数字 " << bestDigit
                              << " 连续" << cfg.digitStableThreshold
                              << "帧稳定，但与目标 " << static_cast<int>(targetNum)
                              << " 不匹配，重置" << std::endl;
                    lastDetectedDigit = -1;
                    consecutiveDigitCount = 0;
                }
