 ...
 *
 * 通信流程（自动识别模式）:
 *   1. 接收 DetectionPacket (0xAA) 获取豆子信息（VisionNode自动识别）
 *   2. 模拟抓取+放置动作序列
 *   3. 接收 NumberPacket (0xCC) 获取数字标签（VisionNode自动识别）
 *   4. 循环
 *
...
int main(int argc, char * argv[])
{
  ...
  while (true) {
    ...
    // 5. 等待数字标签检测（模拟转到数字区后自动识别）
    std::cout << "\n[控制] 转到数字标签区，等待数字检测..." << std::endl;
