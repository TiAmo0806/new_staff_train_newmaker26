...
bool CameraDriver::Open(int cameraIndex)
{
  ...
  // 2. 枚举相机
tSdkCameraDevInfo cameraList[2];
  int cameraCount = 2;
...
cv::Mat CameraDriver::GrabFrame()
{
  ...
  // 分辨率未变时复用预分配缓存
  if (bufferW_ != w || bufferH_ != h)
  {
    frameBuffer_.create(h, w, CV_8UC3);
    bufferW_ = w;
    bufferH_ = h;
  }

  CameraImageProcess(handle_, buffer, frameBuffer_.data, &header);
  cv::cvtColor(frameBuffer_, frameBuffer_, cv::COLOR_RGB2BGR);
