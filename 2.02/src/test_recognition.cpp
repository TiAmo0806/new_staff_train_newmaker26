/**
 * @file test_recognition.cpp
 * @brief 独立识别测试程序 — 脱离状态机和串口，单独测试 YOLO 模型识别效果
 *
 * 功能：
 *   打开相机/图片/视频 → 加载 ONNX 模型 → 实时推理 → 显示检测结果
 *
 * 用途：
 *   在正式比赛前用此程序验证模型的识别效果（豆子种类、数字标签），
 *   可以快速测试不同光照、距离下的识别表现，而不受状态机和串口的影响。
 *
 * 用法:
 *   ./bin/test_recognition                     # 迈德威视工业相机（默认）
 *   ./bin/test_recognition --usb               # USB 摄像头
 *   ./bin/test_recognition --file img.jpg      # 单张图片
 *   ./bin/test_recognition --file video.mp4    # 视频文件
 *   按 'q' 或 ESC 退出
 *
 * @see include/RobotVision.hpp
...
// 类别名称 → 显示文本（全部使用英文，因为 OpenCV Hershey 字体不支持中文）
...
int main(int argc, char* argv[]) {
    ...
    if (access(model_path.c_str(), F_OK) != 0) model_path = "../models/best.onnx";
