...
public:
   ...
   */
  std::vector<uint8_t> ReadFrame(uint8_t expected_header,
  ...
  void SetRxLogEnabled(bool enabled) { rxLogEnabled_ = enabled; }
  bool IsRxLogEnabled() const { return rxLogEnabled_; }

  void SetAutoReconnect(bool enabled) { autoReconnect_ = enabled; }
  bool IsAutoReconnect() const { return autoReconnect_; }
