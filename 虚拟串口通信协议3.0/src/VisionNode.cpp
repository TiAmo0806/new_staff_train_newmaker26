...
// 数字标签模式使用更低阈值（相机已静止，只需消除抖动）
static const int DIGIT_STABLE_THRESHOLD = 3;
...
private:
    ...
    //稳定化有关
    std::vector<DetectionResult> prevDets_;//上一帧检测结果（用于稳定判断）
    int                         stableCount_ = 0;//连续稳定帧数
    bool                        hasSent_ = false;//当前稳定结果是否已发送
    bool                        digitMode_ = false;//true=数字模式, false=豆子模式
    std::atomic<bool>           systemRunning_;//控制线程运行或停止
    std::vector<uint8_t>        cmdBuffer_;//串口命令接收缓冲区（持久化，避免ReadFrame 1ms读不完一帧）
...
void VisionNode::SendDigitDetections(const std::vector<DetectionResult> & dets)
{
    ...
    if (digits.size() == 4)
    {
        ...
        while (it != digits.end() && (static_cast<int>(it->type) - 3) < missingNum)
            ++it;
        digits.insert(it, synth);
        std::cout << "[视觉] 推断缺失数字: " << static_cast<int>(missingNum) << std::endl;
    }
...
int VisionNode::Run()
{
    ...
    while (true)
    {
        ...
        if (!config_.simulate)
        {
            // 根据当前模式筛选：豆子模式只留 BEAN_SOY~KIDNEY，数字模式只留 DATA_1~5
            ...
            {
                if (DetsMatch(filtered, prevDets_))
                {
                    stableCount_++;
                }
                else
                ...
                // 稳定达到阈值且未发送（数字模式用更低阈值）
                int threshold = digitMode_ ? DIGIT_STABLE_THRESHOLD : STABLE_THRESHOLD;
                if (stableCount_ >= threshold && !hasSent_)
                {
                    if (digitMode_)
                        SendDigitDetections(filtered);
                    else
                        SendCombinedDetections(filtered);
                    hasSent_ = true;
                }
            }
        }
        ...
        // 非阻塞读取可用字节，追加到持久缓冲区，然后扫描完整帧
        {
            ...
            if (scanFrame(cmdBuffer_, STATUS_HEADER, STATUS_PACKET_SIZE, cmd_frame))
            {
                ...
                {
                    if (status.flags & FLAG_DIGIT_SCAN)
                    {
                        if (config_.simulate)
                        {
                            // 模拟模式：立即发虚假数字标签
                            std::vector<DetectionResult> simDigits;
                            for (int i = 0; i < 5; ++i)
                            {
                                DetectionResult d;
                                d.type       = static_cast<BeanType>(DATA_1 + i);
                                d.confidence = 90;
                                d.center_x   = -0.8f + i * 0.4f;
                                d.center_y   = 0.0f;
                                d.size       = 0.12f;
                                simDigits.push_back(d);
                            }
                            SendDigitDetections(simDigits);
                        }
                        else if (!digitMode_)
                        {
                            // 真实模式：切到数字模式，等稳定后发
                            std::cout << "[视觉] 切换到数字标签检测模式" << std::endl;
                            digitMode_ = true;
                            stableCount_ = 0;
                            hasSent_ = false;
                            prevDets_.clear();
                    ...
                    else if ((status.flags & FLAG_READY) && digitMode_)
                    {
                        // C板就绪信号 → 切回豆子模式
                        std::cout << "[视觉] 切换到豆子检测模式" << std::endl;
                        digitMode_ = false;
                        stableCount_ = 0;
                        hasSent_ = false;
                        prevDets_.clear();
        ...
        {
            int threshold = digitMode_ ? DIGIT_STABLE_THRESHOLD : STABLE_THRESHOLD;
            std::string modeLabel = digitMode_ ? "数字" : "豆子";
            std::string stableInfo;
            if (config_.simulate)
                stableInfo = "SIM";
            else if (stableCount_ >= threshold)
                stableInfo = "STABLE[" + modeLabel + "]";
            else
                stableInfo = modeLabel + "/观察中(" + std::to_string(stableCount_) + "/" + std::to_string(threshold) + ")";
