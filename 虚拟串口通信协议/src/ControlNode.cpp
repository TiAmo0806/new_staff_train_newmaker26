...
static bool executePickAndPlace(bean_sorter::SerialPort & serial,
                                 ...
                                 const Config & cfg)
{
  uint8_t bin = det.target_bin;
  if (bin < 1 || bin > 5) {
    std::cout << "[控制] 无效料箱号 " << static_cast<int>(bin) << "，跳过" << std::endl;
    return true;
  }

  std::cout << "[控制] 执行: " << bean_sorter::beanTypeName(
    static_cast<bean_sorter::BeanType>(det.bean_type))
    << " -> " << static_cast<int>(bin) << "号箱" << std::endl;

  // Step 1: Move to bean position
  if (!simulateMove(serial, "移动到豆子位置", cfg.moveTimeMs, 0))
    return false;
  ...
  if (!sendStatus(serial, bean_sorter::STATE_COMPLETED,
                  bean_sorter::FLAG_GRIPPER_OPEN, bin,
                  bean_sorter::ERR_NONE)) {
    return false;
  }

  std::cout << "[控制] 抓放完成!" << std::endl;
  return true;
}
...
int main(int argc, char * argv[])
{
  ...
  while (true) {
    ...
    if (det.bean_type == bean_sorter::BEAN_NONE ||
        det.bean_type >= bean_sorter::BEAN_UNKNOWN) {
      std::cout << "[控制] 无效豆种，跳过" << std::endl;
      continue;
    }

    std::cout << "[控制] << "
              << bean_sorter::beanTypeName(static_cast<bean_sorter::BeanType>(det.bean_type))
              << " -> " << static_cast<int>(det.target_bin) << "号箱"
              << " (置信度=" << static_cast<int>(det.confidence) << "%)"
              << " 位置=(" << det.position_x << ", " << det.position_y << ")"
              << std::endl;
